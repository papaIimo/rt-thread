...
BSP_RAM_TEXT_SECTION hi_u32 flash_protect_set_protect(hi_u8 cmp_bp, hi_bool is_volatile, hi_bool forced_write)
{
    ...
    hi_u8 *p2 = &p_data[1];
    flash_protect_lock();
    hi_u32 ret = wait_ready_read_regs(p1, p2);
    if (ret != HI_ERR_SUCCESS) {
        goto end;
    }
...
/* just for wb flash, recovery for erase while power drop */
BSP_RAM_TEXT_SECTION hi_void flash_wb_erase_recovery(const hi_u8 *chip_id)
{
    ...
    flash_protect_lock();
    hi_u32 ret = spif_wait_ready(HI_TRUE, SPI_CMD_SR_WIPN, SPI_SR_BIT_WIP); /* wait WIP set zero */
    if (ret != HI_ERR_SUCCESS) {
        goto end;
    }
    ret = spi_flash_read_reg(SPI_CMD_RDSR3, &reg_val, 1);
    if (ret != HI_ERR_SUCCESS) {
        goto end;
    }
    /* not need to reconfig */
    if (((reg_val >> S19_IDX) & 0x1) == 0x1) {
        goto end;
    }
    reg_val |= (0x1 << S19_IDX);
